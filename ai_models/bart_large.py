...
def load_model_and_tokenizer():
    """
    BART 모델과 토크나이저를 로드하고 모델을 평가 모드로 설정합니다.
...
def generate_summary(model, tokenizer, inputs, max_length, min_length):
    """
...
def summarize_text_with_loaded_model(model, tokenizer, input_text):
    ...
    while True:
        for i in range(0, token_count, stride):
            ...
            summarized_text = generate_summary(model, tokenizer, {'input_ids': input_slice}, max_length, min_length)
